...
class Worker:
    ...
    with sess.as_default(), sess.graph.as_default():
      ...
      try:
        while not coord.should_stop():
          ...
          steps, global_step = self.run_n_steps(self.a, sess)
          self.a=t_max+int(global_step/100000)
  ...
  def run_n_steps(self, n, sess):
    ...
    for _ in range(n):
      ...
      if done:
        ...
        print("Total reward:", self.total_reward, "Worker:", self.name,"Episode Steps:",self.length,"batch:",n)
        self.length=0
  ...
  def update(self, steps, sess):
    ...
    feed_dict = {
      ...
      self.policy_net.p:1/len(steps),
      self.value_net.states: np.array(states),
      self.value_net.targets: value_targets,
      self.value_net.p:1/len(steps)
    }
    #print(1/len(steps))
